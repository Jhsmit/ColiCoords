...
class CellFit(Fit):
    ...
    def __init__(self, cell_obj, data_name='binary', objective=None, minimizer=Powell, **kwargs):
        ...
        super(CellFit, self).__init__(self.model, self.data_elem, minimizer=minimizer, **kwargs)

    def fix_params(self, parameters, **kwargs):
        with set_params(parameters):
            self.execute(**kwargs)
...
class set_params:
    ...
    # todo context manager for fixing / unfixing params
        ...
        fixed_params = [par for par in self.model.params if not par.name in self.parametes]
        for par in fixed_params:
            par.fixed = True
