...
#     intensities = gp.array(intensities, dtype=gp.float16)
#     sigma = gp.array(sigma, dtype=gp.float16)
#     x = gp.array(x, dtype=gp.float16)
#     y = gp.array(y, dtype=gp.float16)
#     img = gp.zeros_like(x_coords)          
#     for _sigma, _int, _x, _y in zip(sigma, intensities, x, y):
#         img += _int * gp.exp(-(((_x - x_coords) / _sigma) ** 2 + ((_y - y_coords) / _sigma) ** 2) / 2)
